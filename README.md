...
1. Clone this repository to your local machine.
  ```bash
  https://github.com/pascutc98/image-classification-CIFAR10
...
5. Explore the `models` directory, which contains two subfolders: `LeNet5/` and `ResNet18/`. Each folder holds the implementation details of its respective neural network architecture.

6. Check the `results` directory for result files in .txt format, including metrics such as true positives, false positives, true negatives, false negatives, recall, precision, specificity, and F1-score. These metrics help assess the performance of the trained models on the test set.

7. The best-performing model weights can be found in the `saved_models` directory in .pth file format.

8. If you want to visualize images from the dataset, explore the `utils` folder. It contains a Python script named `visualize_data.py` that allows you to visualize and optionally save images.
